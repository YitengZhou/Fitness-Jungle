...
After making sure system requirements are met, we can then evaluate the system design based on the extensibility of the project after implementing the design. Firstly, we will go through the list of features we have yet to implement and the foreseeable future enhancements and determining whether we can easily add these features to the system. According to the list, we can easily extend the functionalities of the system. For example, we can easily add new pet skins for each type of pet even though we only have one skin for each type of pet now. That is because we have designed the system based on the “Open-Closed” principle by planning ahead. The data used by all sub-systems are stored in a separate repository, fulfilling the principle of “Separation-of-Concerns” and allowing modifications of each sub-system without affecting dependencies. In addition to this, there is a standardised way of communication through the MQTT regardless of the sub-system. This is achieved by having a consistent format in terms of request and response JSON objects, which act as a common “contract” between different sub-systems. With this infrastructure in place, we can easily substitute each sub-system should we need to. An example will be that we can switch from M5Stack to another type of IoT device and the other sub-systems will not be affected.

The main limitation of our analysis is that we are focusing on the functional aspect of the extensibility and we have not analysed the technical aspect of it. In terms of technical extensibility, code metrics can be used to determine whether the program is written in such a way that we can easily scale. However, again, due to the nature of the project and the objective focusing on the functionalities of the system, we will not be testing our system using code metrics. At the proof-of-concept stage, many things can change thus the important consideration to have a good system architecture in place.
